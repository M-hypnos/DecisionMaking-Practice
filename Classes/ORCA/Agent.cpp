...
namespace RVO {
	...
	size_t Agent::getNearestEnemy(float dist)
	{
		_nearestEnemy = nullptr;
		float rangeSq = sqr(dist);
		sim_->kdTree_->getNearestEnemy(this, rangeSq);
		if (_nearestEnemy == nullptr) return -1;
		else return _nearestEnemy->id_;
	}
		...
		if (this != agent && this->type_ != agent->type_) {
			...
			if (distSq < rangeSq) {
				if (_nearestEnemy == nullptr) {
					...
					if (distSq < absSq(position_ - _nearestEnemy->position_)) {
						_nearestEnemy = agent;
					}
				}
			}
	...
	void Agent::computeNewVelocity()
	{
		...
		for (size_t i = 0; i < agentNeighbors_.size(); ++i) {
			...
			float weightRatio = (other->aiNode_->getWeight() / (aiNode_->getWeight() + other->aiNode_->getWeight()));
			float neighborWeightRatio = (aiNode_->getWeight() / (aiNode_->getWeight() + other->aiNode_->getWeight()));
			Vector2 velocityOpt = (weightRatio >= 0.5f ? (velocity_ - weightRatio * velocity_) * 2 : prefVelocity_ + (velocity_ - prefVelocity_) * weightRatio * 2);
			Vector2 neighborVelocityOpt = (neighborWeightRatio >= 0.5f ? 2 * other->velocity_ * (1 - neighborWeightRatio) : other->prefVelocity_ + (other->velocity_ - other->prefVelocity_) * neighborWeightRatio * 2);
			...
			line.point = velocityOpt + weightRatio * u;
			orcaLines_.push_back(line);
		}
