...
namespace RVO {
	...
	void KdTree::getNearestEnemy(Agent* agent, float& rangeSq) const
	{
		queryAgentTreeRecursiveEx(agent, rangeSq, 0);
	}

	void KdTree::queryAgentTreeRecursiveEx(Agent* agent, float& rangeSq, size_t node) const
	{
		if (agentTree_[node].end - agentTree_[node].begin <= MAX_LEAF_SIZE) {
			for (size_t i = agentTree_[node].begin; i < agentTree_[node].end; ++i) {
				agent->checkNearestEnemy(agents_[i], rangeSq);
			}
		}
		else {
			...
			if (distSqLeft < distSqRight) {
				if (distSqLeft < rangeSq) {
					queryAgentTreeRecursiveEx(agent, rangeSq, agentTree_[node].left);

					if (distSqRight < rangeSq) {
						queryAgentTreeRecursiveEx(agent, rangeSq, agentTree_[node].right);
					}
				}
			}
			else {
